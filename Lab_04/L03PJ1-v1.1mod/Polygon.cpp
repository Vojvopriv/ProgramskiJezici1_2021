...
bool Polygon::operator==(const IObject& other) const {
	const Polygon* r = dynamic_cast<const Polygon*>(&other);
	if (r != nullptr) {
		if (this->numberOfCoordinates != r->numberOfCoordinates)
			return false;

		for (int i = 0; i < this->numberOfCoordinates; i++) {
			if (this->cArray[i] != r->cArray[i])
				return false;
		}
